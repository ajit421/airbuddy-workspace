...
**Roadmap sub-tasks are gone; child nodes are the breakdown.** The `roadmapNodes/{id}/tasks` subcollection and its "Tasks" tab were removed — a milestone is a work item in its own right now, and breaking one down means adding child nodes. `roadmapTaskService.js`, `RoadmapTaskModal` and `RoadmapTaskCard` are consequently unused by the roadmap UI; the Phase 23 mirror-write and the `collectionGroup('tasks')` listener in `TaskContext` still exist for historical data, but there was none at removal time (0 tasks across all 48 nodes).

**A title click opens the detail *and* expands the branch.** In the List view the chevron and the title used to be two separate clicks for what is one question — what is this work, and what is under it. `RoadmapNodeCard`'s title button now calls `onSelect` and then `onToggle`, guarded by `hasChildren && !isExpanded`: expand only, never collapse (hiding a branch with the click that just opened its detail would be a surprise, and the chevron is still how you close one) and never on a leaf (it would open an empty branch and start a pointless `subscribeToChildren` listener). The Journey view is deliberately untouched — its "Explore N sub-quests" is a navigation that replaces the whole path, not an expansion, so folding it into the title click would leave you a level deeper every time you closed a modal.

**On the roadmap page itself, depth decides which surface a click opens.** A root milestone opens `RoadmapNodeDetail`, the right-hand panel — a root is a container, and the panel is where its breadcrumb, comments, attachments, history and **Add Child** live. A child node opens `TaskDetailModal` through [src/components/Roadmap/RoadmapNodeTaskModal.jsx](src/components/Roadmap/RoadmapNodeTaskModal.jsx), because a child *is* the unit of work. `handleSelect` in `CompanyRoadmap` is the single funnel for both List and Journey views (including the Journey drill-down), so the branch lives there and nowhere else. Two details are load-bearing: opening a child deliberately does **not** clear `selectedNodeId` — the panel was showing the *root* the user picked, and clearing it would make the modal's own Company Roadmap link a no-op navigation to the URL the page is already on for a deep-linked child, leaving comments/history/Add Child unreachable. And a deep link always opens the panel whatever the depth, which is precisely what that link relies on. `nodeToWorkItem` is what adapts the node, so the modal reaches `roadmapNodes` rather than `tasks`; the wrapper holds the single-document listener so a node snapshot re-renders one modal instead of the whole tree, and seeds its first render from the tree's own snapshot so nothing flashes empty.

**`nodeToWorkItem` passes `workPartners`, `workPartnerUids`, `attachments` and `isExtended` straight through, and hardcoding them empty was a bug.** `TaskDetailModal` renders all four off that object and `permissions.isWorkPartner()` reads `workPartnerUids` to decide who may tick a todo — while the rules carve-out on `roadmapNodes` and `NODE_ASSIGNEE_WRITABLE_FIELDS` mean a node genuinely carries all four. Empty values meant a work partner added to a milestone vanished on reopen, Extend left no "Extended" badge, and the completion dialog's `[...(task.attachments || []), attachment]` wrote back an array holding only the newest attachment. They still default to `[]`/`false`, so a bare node reads as "none".

**A milestone's detail panel is the ordinary `TaskDetailModal`, deliberately identical to a task's** — same todo list, work partners, collaboration timeline and Extend control. That works because the shared services resolve their collection through [src/utils/workItemRef.js](src/utils/workItemRef.js) instead of hardcoding `tasks`: `todoService` and `collaborationService` take the **work item object**, not a bare id, and `workItemCollection()` maps `_source: 'roadmapNode'` onto `roadmapNodes`. Two things follow. `useTaskTodos` and `useTaskTimeline` key their effect on a derived `collection/id` **string**, never on the object — depending on the object would tear down and rebuild the snapshot listener on every render. And `firestore.rules` mirrors the task rules onto `roadmapNodes`: an assignee carve-out for exactly what the modal writes (`status, progress, completionNote, attachments, dueDate, isExtended, updatedBy, updatedAt` — keep it identical to `NODE_ASSIGNEE_WRITABLE_FIELDS`), a separate **participant** carve-out for the shared arrays (`workPartners, workPartnerUids, todos`) so a work partner can tick a checklist item without gaining the right to set progress, and a `roadmapNodes/{id}/events` subcollection matching `tasks/{id}/events`. The one intentional difference from a task is that **Delete is hidden**: removing a milestone is `archiveNode`, because a task-style delete would orphan its subtree.
