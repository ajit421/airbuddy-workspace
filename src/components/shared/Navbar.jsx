...
export default function Navbar({ onMenuToggle }) {
  ...
  return (
    <header className="h-16 bg-surface border-b border-border flex items-center justify-between px-4 lg:px-6 sticky top-0 z-40">
      ...
      {/* Right side */}
      <div className="flex items-center gap-2">
        {/* Docs — icon-only below sm so it stays reachable on phones */}
        <Link
          to="/docs"
          className="flex items-center justify-center sm:justify-start gap-1.5 w-9 h-9 sm:w-auto sm:h-auto sm:px-3 sm:py-1.5 rounded-lg text-xs font-semibold text-text-muted hover:text-text-primary hover:bg-surfaceHover transition-all border border-border hover:border-orange/40"
          title="Documentation"
        ...
        {/* View Mode Toggle — Card ↔ Table.
            p-0 is required: btn-ghost carries px-3 py-2, which against a fixed w-8 h-8
            leaves a ~6px content box and squashes the icon. */}
        <button
          id="view-mode-toggle"
          onClick={toggleViewMode}
          title={viewMode === 'card' ? 'Switch to Table view' : 'Switch to Card view'}
          aria-label={viewMode === 'card' ? 'Switch to Table view' : 'Switch to Card view'}
          className={`flex items-center justify-center w-9 h-9 sm:w-8 sm:h-8 p-0 rounded-lg transition-all border ${viewMode === 'table'
